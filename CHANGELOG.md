...
- **Verified broken:** One validation request timed out when the first post-trim guard regenerated multiple sections through legacy model calls; fixed by restoring structurally valid untrimmed batched content instead of adding extra model calls. Checked: parser fallback still feeds missing, short, or structurally incomplete body sections into `runSection`; `runSection` still performs Rule-5 lint/repair, Rule-6 contradiction repair, placeholder stripping, stitching, references injection, FAQ top-up, Quick Tips fallback, internal links, and sanitisation. Healthcare-clinical body generation remains excluded from body batching.
