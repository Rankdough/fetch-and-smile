## 2026-06-11 — Phase 1: batched prompt generation behind USE_BATCHED_PROMPT flag (V2P1)

- **What:** Added `generateBodyBatch` and `generateTailBatch` paths to `proprietary-generate-article/index.ts`. When `USE_BATCHED_PROMPT=true`, body sections (N sections, 1 API call) and framing sections (M sections, 1 API call) are generated in two batch calls instead of N+M individual calls. Context files and rule text are sent once per batch rather than once per section. Per-section fallback fires for any section the parser fails to extract. New shared file `_shared/proprietaryStaticPrompt.ts` exports `buildStaticPromptBlock` (article-level invariant prompt parts) and `assertStaticBlockByteParity` (runtime byte-identity assertion). Feature flag is `false` by default — with flag off, code path is byte-identical to the pre-V2P1 `fix/a1-tldr` baseline.
- **Why:** Per-article input-token cost was dominated by context file and rule text replay: each of 7-9 section calls re-sent the full context files (10-30 KB each) and ~5 KB of rule text. Batching removes this repetition and is projected to reduce input tokens by ≥40% for typical articles.
- **What may break:** With flag on: (1) surrounding context is not threaded through batch-generated sections (all sections generated in parallel); (2) healthcare-clinical body sections are excluded from batching and continue to use the clinical per-section writer — the tail batch still applies; (3) parser miss on any section triggers a per-section fallback call (logged as BATCH FALLBACK). No change to any post-processing step (Rule-5 lint, Rule-6 repair, references injection, CTA injection, stitching, schema wrapping). Flag-off path is completely unchanged.
- **Files:** `supabase/functions/proprietary-generate-article/index.ts`, `supabase/functions/_shared/proprietaryStaticPrompt.ts`, `CHANGELOG.md`.
- **Verify steps:** (1) Generate with `USE_BATCHED_PROMPT=false` and confirm BUILD_MARKER `BUILD-2026-06-11-V2P1-batched-prompt` in boot log. (2) Diff against pre-change article output — must be zero. (3) Set `USE_BATCHED_PROMPT=true`, generate 5 articles (mix of healthcare-clinical + jersey/sports), run Rule Checklist. (4) Check boot log for `ARTICLE_TELEMETRY` JSON with `batched: true` and `input_tokens` field. Compare to `batched: false` run on same topic.
