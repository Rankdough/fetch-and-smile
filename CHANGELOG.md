...
- **What:** Changed `proprietary-generate-article` so the v2 batched generator is now the default path instead of dormant. `USE_BATCHED_PROMPT=false` remains an instant backend rollback. Added request override support via `flags.useBatchedPrompt`. Added batched framing generation so Opening, TL;DR, Quick Tips, FAQ, and Final Thoughts are produced in one call instead of five separate section calls. Non-clinical body sections still generate in one batched call and healthcare-clinical body sections still use the clinical writer path. Added body-section acceptance guards before and after trimming so batched sections with missing 4-row tables or missing required bullets fall back to the legacy per-section path. Bumped `BUILD_MARKER` to `BUILD-2026-06-11-V2P1-default-batched-posttrim`.
- **Why:** The previous change was deployed with batching off, so proprietary generation stayed as slow and token-heavy as before. This makes the cost-saving path active while preserving parser fallback and existing post-processing.
- **Verified broken:** Nothing verified broken. Checked: parser fallback still feeds missing, short, or structurally incomplete body sections into `runSection`; `runSection` still performs trim, Rule-5 lint/repair, Rule-6 contradiction repair, placeholder stripping, stitching, references injection, FAQ top-up, Quick Tips fallback, internal links, and sanitisation. Healthcare-clinical body generation remains excluded from body batching.
- **Files:** `supabase/functions/proprietary-generate-article/index.ts`, `supabase/functions/_shared/proprietaryBatchedPrompt.ts`, `CHANGELOG.md`.
- **Verify:** Deploy `proprietary-generate-article`; confirm boot log shows `BUILD-2026-06-11-V2P1-default-batched-posttrim` and `PROPRIETARY BATCH MODE: batched`. Generate one proprietary service article and confirm logs show `BATCHED BODY`, `BATCHED FRAMING`, and `PROPRIETARY TELEMETRY` with non-zero `batched_prefilled` and `framing_batched_prefilled`.

## 2026-06-11 — V2P1 batched body generation (flag-gated, default off)

- **What:** Phase 1 of the proprietary v2 rebuild. Adds a new shared module `supabase/functions/_shared/proprietaryBatchedPrompt.ts` (`buildBatchedBodyPrompt` + `parseBatchedSections`) and wires it into `supabase/functions/proprietary-generate-article/index.ts` behind a `USE_BATCHED_PROMPT` env flag. When the flag is `"true"` and the article is non-clinical, all body H2 sections (h2-question + failure-mode + how-to-choose) are generated in a SINGLE model call instead of one call per section. Per-section parse failures fall back to the legacy `runSection` path for just that section. Per-section RAG retrieval, mapped-unit pick, and allowed-source-URL pool are computed once (in parallel during the pre-pass) and cached so the main loop does not re-fetch. Framing sections (Opening, TL;DR, Quick Tips, FAQ, Final Thoughts) and the healthcare-clinical writer path are UNCHANGED in Phase 1. Bumped `BUILD_MARKER` to `BUILD-2026-06-11-V2P1-batched-prompt`. Added per-article telemetry log line `PROPRIETARY TELEMETRY: { batched_flag, batched_attempted, body_sections_total, batched_prefilled, batched_fallback_ids, batched_raw_length, business_type }`.
- **Why:** With ~8 body sections per article, the per-section loop re-sends the static prompt (tone profile + value promises + context files + rules 1-16) 8 times. Batching collapses that to one shared static block + N small per-section briefs, targeting a ~40-60% input-token reduction.
- **What may break:** Nothing when `USE_BATCHED_PROMPT` is unset or `"false"` — the new branch in `runSection` is gated on `preGeneratedContent` being non-empty (default undefined), the pre-pass block only runs when the flag is on, and the loop's cache check defaults to computing data exactly as before. With the flag ON: (a) the batched model output uses `<<<SECTION id="..." heading="..." budget_words=...>>>...<<<END SECTION>>>` delimiters — if the model deviates, that section's id is added to `batched_fallback_ids` and the legacy per-section call runs for it, so the article still ships; (b) batched sections lose the `surroundingContext` chaining (each section is written in parallel inside one prompt); the model still sees every section's brief and the global rules.
- **Files:**
  - new: `supabase/functions/_shared/proprietaryBatchedPrompt.ts`
  - changed: `supabase/functions/proprietary-generate-article/index.ts` — added `USE_BATCHED_PROMPT` env read, `preGeneratedContent?` field on `runSection`, `computeSectionData` helper, batched pre-pass block, cache-aware loop, telemetry log, bumped `BUILD_MARKER`.
  - changed (unrelated pre-existing build errors fixed to clear the build gate): `src/components/keyword-research/ContentQueue.tsx` (extra `</div>`), `src/pages/ShopifyFaqBulk.tsx` (`row` → `r`), `src/utils/generateMigrationArticle.ts` (`contextFiles` → `extraContextFiles`).
- **Verify:**
  1. Confirm boot log shows `BUILD-2026-06-11-V2P1-batched-prompt proprietary-generate-article USE_BATCHED_PROMPT= false` on the next proprietary call (default off).
  2. Generate one proprietary article with no env change. Output should be identical to current behaviour. Telemetry line should show `batched_flag: false, batched_attempted: false`.
  3. Set `USE_BATCHED_PROMPT=true` on the project. Generate the jersey-stitching test topic. Boot log shows `USE_BATCHED_PROMPT= true`. Telemetry should show `batched_attempted: true, batched_prefilled: <N>, batched_fallback_ids: []` ideally. Run the CLAUDE.md Rule Checklist on the output.
  4. Compare avg input tokens via the Lovable AI gateway dashboard between flag-off and flag-on runs of the same topic.
