...
- **Verified broken:** Nothing verified broken. Checked: parser fallback still feeds missing or short sections into `runSection`; `runSection` still performs trim, Rule-5 lint/repair, Rule-6 contradiction repair, placeholder stripping, stitching, references injection, FAQ top-up, Quick Tips fallback, internal links, and sanitisation. Healthcare-clinical body generation remains excluded from body batching.
