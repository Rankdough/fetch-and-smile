...
// C5: Strip hallucinated Resources sections injected by insert-internal-links Gemini.
// The model sometimes adds a "### Resources" or "## Resources" section when an
// off-topic URL slug token (e.g. "jersey") matches unrelated URLs (e.g. baseball).
// Safe to strip when the body contains "For more information, explore additional resources"
// or similar boilerplate — these are never part of the article template.
function stripHallucinatedResourcesSections(markdown: string): { out: string; stripped: boolean } {
  let stripped = false;
  const out = markdown.replace(
    /\n+(#{2,3})\s+Resources?\s*\n([\s\S]*?)(?=\n#{1,3}\s+|$)/gi,
    (match, _hashes, body) => {
      if (/for more information|explore additional resources|additional resources/i.test(body)) {
        stripped = true;
        console.log(`HALLUCINATION GUARD: stripped off-topic Resources section.`);
        return "\n";
      }
      return match;
    },
...
// C5: Strip boilerplate FAQ items generated by Gemini when it runs out of real questions.
// Detected patterns: "What is the main point of..." / "How should someone use this information about..."
// Stripping these before ensureFiveFaqPairs allows it to fill with content-derived pairs instead.
function stripBoilerplateFaqItems(faqContent: string): { out: string; stripped: number } {
  let stripped = 0;
  const BOILERPLATE_Q_RE = /\*\*(?:What is the (?:main point|key takeaway|primary purpose|primary focus) of [^*\n]+|How should someone use this information about [^*\n]+)\??\*\*\s*\n+(?:[^\n*]|\n(?!\s*\n|\*\*))*\n*/gi;
  const out = faqContent.replace(BOILERPLATE_Q_RE, () => {
    stripped++;
    return "";
  });
  if (stripped > 0) console.warn(`FAQ BOILERPLATE GUARD: stripped ${stripped} generic Q&A pair(s).`);
  return { out: out.replace(/\n{3,}/g, "\n\n").trim(), stripped };
}
  ...
  try {
    ...
    for (const s of sectionsOut) {
      ...
      if (s.kind === "opening") {
        ...
        // C5: strip boilerplate "What is the main point of..." / "How should someone use..."
        // pairs Gemini generates when it runs out of real questions — before top-up so
        // ensureFiveFaqPairs fills with content-derived pairs from body sections instead.
        const faqBoilerplate = stripBoilerplateFaqItems(cleanContent);
        // Enforce EXACTLY 5 Q&A pairs: top-up with deterministic fillers if model produced fewer.
        const topped = ensureFiveFaqPairs(faqBoilerplate.out, body.topic, sectionsOut);
    ...
    // C5: strip unquantified hedge words (Rule 7 compliance — target ≤2 per article).
    const hedgeResult = stripUnquantifiedHedges(stitched);
    stitched = hedgeResult.out;
    if (hedgeResult.stripped > 0) console.warn(`HEDGE GUARD: stripped ${hedgeResult.stripped} unquantified hedge word(s).`);
    ...
    } else if (contextFileNames.length > 0) {
      // C5: Always append source document file names to the References section so
      // readers can trace which files were consulted, even when URL refs were found.
      const refIdx = stitched.search(/^##\s+references/im);
      const afterRef = stitched.slice(refIdx);
      const alreadyPresent = contextFileNames.some((name) =>
        afterRef.toLowerCase().includes(name.toLowerCase())
      );
      if (!alreadyPresent) {
        const existingNums = (afterRef.match(/^\d+\./gm) || []).length;
        const startN = existingNums + 1;
        const fileLines = contextFileNames.map((name, i) => `${startN + i}. ${name}`).join("\n");
        stitched = stitched.trimEnd() + "\n" + fileLines + "\n";
        console.log(`REFERENCES: appended ${contextFileNames.length} source file name(s) to References section.`);
      }
    }
    ...
    // C5: strip hallucinated Resources sections added by insert-internal-links Gemini
    // (fires when an off-topic URL slug token matches the article topic — e.g. "jersey" → baseball).
    const resourcesStrip = stripHallucinatedResourcesSections(content);
    if (resourcesStrip.stripped) content = resourcesStrip.out;
